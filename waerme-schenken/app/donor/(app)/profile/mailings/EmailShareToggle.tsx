...
export default function EmailShareToggle({ initialConsent }: { initialConsent: boolean }) {
    ...
    async function toggleConsent() {
        ...
        try {
            const res = await fetch('/api/users/me', {
                ...
                body: JSON.stringify({ emailShareConsent: newConsent }),
            });
            ...
            <div className="flex items-center justify-between p-5 rounded-2xl bg-gray-50 border border-gray-100">
                <div>
                    ...
                    <p className="text-sm opacity-60 flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${consent ? 'bg-green-400' : 'bg-red-400'}`}></span>
                        {consent ? 'Freigegeben' : 'Nicht freigegeben'}
                    </p>
                </div>
                ...
                >
                    {loading ? 'Laden...' : (consent ? 'Widerrufen' : 'Zustimmen')}
                </button>
            </div>

            <p className="text-[11px] opacity-40 text-center mt-4 px-4">
                {consent
                    ? 'Familien dürfen deine E-Mail-Adresse sehen, um sich zu bedanken.'
                    : 'Klicke auf Zustimmen, um Familien zu erlauben, dich per E-Mail zu kontaktieren.'}
            </p>
        </div>
    );
}
